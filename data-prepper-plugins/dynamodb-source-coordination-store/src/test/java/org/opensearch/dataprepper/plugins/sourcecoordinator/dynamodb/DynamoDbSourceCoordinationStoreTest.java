...
public class DynamoDbSourceCoordinationStoreTest {
    ...
    void tryUpdateSourcePartitionItem_calls_dynamoClientWrapper_correctly() {
        ...
        given(dynamoDbClientWrapper.tryUpdatePartitionItem((DynamoDbSourcePartitionItem) updateItem)).willReturn(true);

        final boolean result = createObjectUnderTest().tryUpdateSourcePartitionItem(updateItem);
    ...
    void tryAcquireAvailablePartition_iterates_until_it_successfully_acquires_a_partition() {
        ...
        given(dynamoDbClientWrapper.tryUpdatePartitionItem(itemList.get(0))).willReturn(false);
        given(dynamoDbClientWrapper.tryUpdatePartitionItem(itemList.get(1))).willReturn(true);
    ...
    void tryAcquireAvailablePartition_returns_empty_optional_when_an_exception_is_thrown_while_iterating(final Class exception) {
        ...
        given(dynamoDbClientWrapper.tryUpdatePartitionItem(itemList.get(0))).willReturn(false);
        given(dynamoDbClientWrapper.tryUpdatePartitionItem(itemList.get(1))).willThrow(exception);
