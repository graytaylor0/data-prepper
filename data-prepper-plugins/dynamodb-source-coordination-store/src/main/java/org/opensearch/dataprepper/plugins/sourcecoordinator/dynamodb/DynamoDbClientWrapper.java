...
public class DynamoDbClientWrapper {
    ...
    public boolean tryUpdatePartitionItem(final DynamoDbSourcePartitionItem dynamoDbSourcePartitionItem) {
        try {
            ...
            LOG.warn("ConditionalCheckFailed while updating partition {}. " +
                            "This partition item was either deleted from the dynamo table, or another instance of Data Prepper has modified it: ",
                    dynamoDbSourcePartitionItem.getSourcePartitionKey(), e);
            return false;
        } catch (final ProvisionedThroughputExceededException e) {
            LOG.error("Unable to update partition item for {} in DynamoDb due to throttling even after exponential backoff and retry {}", dynamoDbSourcePartitionItem.getSourcePartitionKey(), e.getMessage());
            return false;
        } catch (final Exception e) {
            LOG.error("An exception occurred while attempting to update a DynamoDb partition item {}", dynamoDbSourcePartitionItem.getSourcePartitionKey(), e);
            return false;
        }
    }
